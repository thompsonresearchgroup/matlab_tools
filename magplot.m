function [] = magplot(file,scale,cscale,vscale)

%file is a .xyz file of the molecule augmented with atom centered magnetic moments
%scale sets the diameter of the atom spheres
% cscale sets the diameter of the connecting cylinders

fid = fopen(file);
xyz = textscan(fid,' %s %f %f %f %f %f %f', 'delimiter',' ','MultipleDelimsAsOne',1,'HeaderLines', 2);
fclose(fid);
...
quiver3D([AtX(:),AtY(:),AtZ(:)],vscale*[VecX(:),VecY(:),VecZ(:)],'r')
hold off
daspect([1 1 1]);
view([13.0326   12.9628    8.6489]);
camlight('right');
lighting gouraud;
